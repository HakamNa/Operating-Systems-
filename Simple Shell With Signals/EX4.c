...
pid_t ctrl_z_handler = 0; // a global pid to save the child id for the last command when the user enters Ctrl-z
pid_t pid_for_child = 0; // pid to do the main order and to activate the commands
...
// functions structure
void sig_handler(int sig) // placing value
{
    ctrl_z_handler = pid_for_child; // saving the id after clicking Ctrl-z
}
...
int main() {
    signal(SIGTSTP, sig_handler); // handling the case of entering Ctrl-z
    ...
    while (read_parse_line(args, line) && end != true) {
        ...
        if (general_pipe == 0) // if no pipes have been found
        {
            if (strcmp(args[0], "fg") == 0 && args[1] == NULL) // when the user enter fg ,continue the work
            {
                kill(ctrl_z_handler, SIGCONT); //send a signal to a process or a group of processes specified by pid  
                waitpid(ctrl_z_handler, NULL, WUNTRACED); // waiting for the child until it ends or entering Ctrl-z

            } else {
                pid_for_child = fork();
                if (pid_for_child == -1) // if the fork return -1, throw error
                {
                    perror("System Call Failed");
                    exit(1);
                } else if (pid_for_child == 0) // if the fork return 0 , im in child
                {
                    signal(SIGTSTP, SIG_DFL); // default of ^z
                    // im in child
                    ...
                    // im in parent
                    waitpid(pid_for_child, NULL, WUNTRACED); // wait the child till it ends
                }
            }
        }
